            ...
            <form action="{{ route('programs.store') }}" method="POST" class="max-w-2xl">
                ...
                <div class="mb-6">
                    ...
                    <select
                        ...
                        <option value="{{ $kategori['id'] }}" {{ old('kategori_program_id') == $kategori['id'] ? 'selected' : '' }}>
                            {{ $kategori['title'] }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    <x-forms.input label="Target Donasi" name="target_amount" type="number" value="{{ old('target_amount') }}" required />
                </div>

                <div class="mb-4">
                    <x-forms.input label="Tanggal Mulai" name="start_date" type="date" value="{{ old('start_date', $start_date) }}" required />
                </div>

                <div class="mb-4">
                    <x-forms.input label="Tanggal Berakhir" name="end_date" type="date" value="{{ old('end_date', $end_date) }}" required />
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select
                        ...
                        <option value="0" {{ old('is_priority') == '0' ? 'selected' : '' }}>Tidak</option>
                        <option value="1" {{ old('is_priority') == '1' ? 'selected' : '' }}>Ya</option>
