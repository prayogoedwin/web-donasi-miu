<x-layouts.app>
    ...
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Program</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">Manage System Program</p>
        </div>
        <div class="flex gap-2">
            @if(auth()->user()->hasPermission('download-programs'))
            @if (($canDownload ?? true) !== false)
            <a href="{{ route('programs.export') }}">
            ...
            @endif
            @endif
            @if(auth()->user()->hasPermission('create-programs'))
            @if (($canCreate ?? true) !== false)
            <a href="{{ route('programs.create') }}">
                <x-button type="primary">{{ __('Create Program') }}</x-button>
            </a>
            @endif
            @endif
