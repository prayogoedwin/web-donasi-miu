<x-layouts.app>
    <div class="mb-6 flex items-center text-sm">
        ...
        <span class="text-gray-500 dark:text-gray-400">Informasis</span>
    </div>

    <div class="mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Informasis</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">Manage System Informasis</p>
        </div>
        <div class="flex gap-2">
            @if(auth()->user()->hasPermission('download-informasis'))
            @if (($canDownload ?? true) !== false)
            <a href="{{ route('informasis.export') }}">
            ...
            @endif
            @endif
            @if(auth()->user()->hasPermission('create-informasis'))
            @if (($canCreate ?? true) !== false)
            <a href="{{ route('informasis.create') }}">
                <x-button type="primary">{{ __('Create Informasi') }}</x-button>
            </a>
            @endif
            @endif
            ...
            <table id="dynamic-table" class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Bagian Dari</th>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#dynamic-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('informasis.indexTable') }}",
                columns: [
                    { data: 'key', name: 'key' },
                ...
                language: {
                    search: "_INPUT_",
                    searchPlaceholder: "Search Informasis",
                    lengthMenu: "Show _MENU_ entries",
                    info: "Showing _START_ to _END_ of _TOTAL_ Informasis",
                    infoEmpty: "No Informasis found",
                    infoFiltered: "(filtered from _MAX_ total Informasis)",
                    zeroRecords: "No matching Informasis found",
                    emptyTable: "No Informasis available"
                },
